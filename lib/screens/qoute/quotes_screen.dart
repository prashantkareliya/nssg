...
class _QuoteScreenState extends State<QuoteScreen> {
  ...
  AnimatedOpacity buildSearchBar(BuildContext context) {
    return AnimatedOpacity(
      ...
      duration: const Duration(milliseconds: 500),
      child: Visibility(
        visible: Provider.of<WidgetChange>(context, listen: true).isAppbarShow ? false : true,
        child: Padding(
          padding: EdgeInsets.only(right: 24.sp, top: 8.sp, left: 15.sp, bottom: 0),
          child: Padding(
            ...
            child: Row(
              children: [
                InkWell(
                    ...
                    child: Icon(Icons.arrow_back_ios_rounded, color: AppColors.blackColor)),
                SizedBox(width: 5.w),
                Expanded(
                    ...
                    child: AnimationLimiter(
                      child: ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          String name = "";
                          if (quoteItems![index].contactName != null) {
                            if (quoteItems![index].contactName!.contains(" ")) {
                              if (quoteItems![index].contactName!.split(" ").length > 1) {
                                if (quoteItems![index].contactName!.split(" ")[1].isNotEmpty) {
                                  name =
                                      "${quoteItems![index].contactName!.split(" ")[0][0]}${quoteItems![index].contactName!.split(" ")[1][0]}"
                                          .toUpperCase();
                                } else {
                                  name = quoteItems![index].contactName![0].trim().toUpperCase();
                                }
                              } else {
                                name = quoteItems![index].contactName![0].trim().toUpperCase();
                              }
                                              ...
                                              onPressed: (value) {
                                                if (searchKey.isNotEmpty) {
                                                  callNextScreen(
                                                      context,
                                                      AddJobPage(
                                                          quoteItem: searchItemList![index]));
                                                } else {
                                                  callNextScreen(context,
                                                      AddJobPage(quoteItem: quoteItems![index]));
                                                }
                                              },
...
class _QuoteDetailState extends State<QuoteDetail> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        appBar: BaseAppBar(
            appBar: AppBar(),
            title: widget.quoteData?.quoteNo,
                                    ...
                                    children: [
                                      Column(
                                        ...
                                        children: [
                                          // Text("Quote no : ",
                                          //     style:
                                          //         CustomTextStyle.commonStyle),
                                          // Text(dataQuote["quote_no"],
                                      ...
                                      child: SizedBox(
                                        ...
                                        child: TextButton(
                                            ...
                                            onPressed: () {
                                              print("Generate job");
                                              callNextScreen(
                                                  context, AddJobPage(quoteItem: widget.quoteData));
                                            },
                                            child: Icon(
                                              Icons.sync_rounded,
                                              color: AppColors.primaryColor,
